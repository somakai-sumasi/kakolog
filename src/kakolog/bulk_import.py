...
def bulk_import(claude_projects_dir: Path | None = None):
    ...
    jsonl_files = sorted(
        f for f in claude_projects_dir.rglob("*.jsonl") if "subagents" not in f.parts
    )
    ...
    with connection() as conn:
        existing = {
            r[0]
            for r in conn.execute("SELECT DISTINCT session_id FROM memories").fetchall()
        }
        ...
        for i, jsonl_path in enumerate(jsonl_files):
            ...
            for chunk, emb in zip(chunks, embeddings):
                if not touch_if_exists(
                    conn, chunk.question, chunk.answer, project_path
                ):
                    insert_memory(
                        conn,
                        session_id,
