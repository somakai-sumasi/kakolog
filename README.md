...
[Claude Code] ──MCP (streamable-http)──> [kakolog-mcp :7377]
                                            ...
                                            └── POST /hook/save  (UserPromptSubmit/SessionEndフック専用)
...
- **UserPromptSubmit/SessionEndフック**: バックグラウンドcurlで `/hook/save` に転送。`UserPromptSubmit`で毎ターン保存しターミナル強制終了に備え、`SessionEnd`で`/new`・通常終了時に最終保存
