...
- **検索 (`search`)**: FTS5 + ベクトル検索 → RRF統合 → スコープフィルタ → MMR多様性選択 → 結果を返す（`project_path`/`session_id`で絞り込み可能、`use_rerank=True`でcross-encoderリランキングも可能）
- **保存 (`save`)**: チャンク分割→ベクトル化→完全一致/類似度重複チェック→DB保存（類似度≥0.96の既存データはスキップしアクセス追跡を更新）
...
RRF (Reciprocal Rank Fusion)
  ...
  │
  v
スコープフィルタ (SearchScope)
  │   └── project_path / session_id で絞り込み (オプション)
  │
  v
...
- トリビアル入力 (`y`, `ok`, `続けて` 等)
- 重複チャンクの排除 (完全一致: 同一content+project_pathは1件のみ保存、類似度: 同一project_path内でコサイン類似度≥0.96の既存データはスキップしaccess_count/last_accessed_atを更新)
...
| テスト | pytest + pytest-cov | ユニットテスト69件 |
...
│   ├── save.py     # 保存サービス (重複スキップ+類似度チェック+保存)
...
├── config.py       # ユーザー設定 (~/.kakolog/config.json, 除外パス管理, 類似度閾値)
└── migrations/
