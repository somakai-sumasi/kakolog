...
| モード | 速度 | 精度 | 用途 |
|--------|------|------|------|
| デフォルト (FTS5 + vec → RRF) | ~15ms | 中 | 通常の検索 |
| リランキング有効 (`use_rerank=True`) | ~400ms | 高 | 精度が必要な時 |

FTS5とベクトル検索はそれぞれ異なる強みを持つ。FTS5は固有名詞やエラーコードの完全一致に強く、ベクトル検索は意味的な類似性を捉える。リランキングはオプションで、`use_rerank=True`を指定するとCross-Encoderがクエリと文書を直接比較して最終スコアを決定する。
