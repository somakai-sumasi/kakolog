...
[Claude Code] --MCP(http:7377)--> [kakolog-mcp (launchd常駐)]
                                      ...
                                      └── POST /hook/save     (UserPromptSubmit/SessionEndフック専用HTTP endpoint)

[UserPromptSubmit/SessionEnd hook] --curl--> [kakolog-mcp :7377/hook/save]
```
...
hooks/
├── save-on-session-end.sh  # UserPromptSubmit/SessionEndフック (バックグラウンドcurlでMCPサーバーに転送)
...
`UserPromptSubmit`フックでユーザーがメッセージを送信するたびに前ターンまでの完了済み会話を保存し、ターミナル強制終了時のデータ欠損を防ぐ。`SessionEnd`は`/new`・通常終了・exit時に発火する最終保存。curlはバックグラウンド実行（`&`）なのでClaudeの応答をブロックしない。
...
- UserPromptSubmit/SessionEndフックはバックグラウンドcurl（`&`）で `/hook/save` に転送。Python直接起動だとtransformersのimportで1秒以上かかりhookタイムアウト(SIGINT)に引っかかるため
- UserPromptSubmitは前ターンの完了済み会話のみ処理するため、Stopフックのようなターン途中の重複チャンクは発生しない
