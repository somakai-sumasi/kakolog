...
[Claude Code] --MCP(http:7377)--> [kakolog-mcp (launchd常駐)]
                                      ...
                                      └── POST /hook/save     (Stop/SessionEndフック専用HTTP endpoint)

[Stop/SessionEnd hook] --curl--> [kakolog-mcp :7377/hook/save]
```
...
hooks/
├── save-on-session-end.sh  # Stop/SessionEndフック (バックグラウンドcurlでMCPサーバーに転送)
...
`Stop`フックで毎ターン保存することで、`/new`やターミナル強制終了時のデータ欠損を防ぐ。`SessionEnd`は最終フォールバック。curlはバックグラウンド実行（`&`）なのでClaudeの応答をブロックしない。
...
- Stop/SessionEndフックはバックグラウンドcurl（`&`）で `/hook/save` に転送。Python直接起動だとtransformersのimportで1秒以上かかりhookタイムアウト(SIGINT)に引っかかるため
- Stopフックは毎ターン発火するが、`touch_if_exists`で既存チャンクはスキップされるため重複処理は最小限
